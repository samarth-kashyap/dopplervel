"""Compute spherical harmonic time series from interpolated velocity maps.

This module performs spherical harmonic transforms on interpolated velocity
maps to obtain time series of spherical harmonic coefficients. It computes
the radial (u), poloidal (v), and toroidal (w) components of the velocity
field in the spectral domain.

The transforms use associated Legendre polynomials from pyshtools and
...
def gen_leg(lmax, theta):
    """Generate associated Legendre polynomials and their derivatives.

    Computes normalized associated Legendre polynomials P_l^m and their
    theta-derivatives for spherical harmonic transforms.
    ...
    theta : np.ndarray(ndim=1, dtype=float)
        1D array of colatitude values [radians]

    Returns
    -------
    leg : np.ndarray(ndim=2, dtype=float)
        Associated Legendre polynomials, shape (maxIndex, len(theta))
    dt_leg : np.ndarray(ndim=2, dtype=float)
        Theta-derivatives of Legendre polynomials, normalized by
        sqrt(2*ell*(ell+1) - m*(2*ell + 1))
    """
    cost = np.cos(theta)
    sint = np.sin(theta)
